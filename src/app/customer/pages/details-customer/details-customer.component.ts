...
export class DetailsCustomerComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.detailService.getCustomer(param).subscribe((customer: Customer) => {
      this.user = customer;
      this.detailService
        .getAccountsCustomer(customer.id)
        .subscribe((accounts: Account[]) => {
          this.accounts = accounts;
        });
      this.detailService
        .getLoansCustomer(customer.id)
        .subscribe((loans: Credit[]) => {
          this.loans = loans;
        });
      this.detailService
        .getCreditCardsCustomer(customer.id)
